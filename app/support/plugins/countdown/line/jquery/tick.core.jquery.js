...
var serverDate = function serverDate(cb) {
	...
	var now = Date.now();
	xhr.open('HEAD', location + '?noCache=' + now);
...
var Views = {
	'text': function text() {
		return function (root) {
			...
			drawer(state, function (state) {
				state.root.setAttribute('data-value', state.value);
				state.root.textContent = state.value;
			});

			return Object.assign({}, rooter(state, root, 'text'), updater(state), destroyer(state));
		};
	}
...
var getNextScheduledDate = function getNextScheduledDate(date, schedule) {
	var dist = null;
	return schedule.split(',').map(trim).map(function (s) {
		return toMoment(date, s);
	}).filter(function (moment) {
		return moment !== null && moment.date !== null;
	}).reduce(function (nearest, moment) {
		dist = moment.dist;
		if (!nearest || moment.dist < dist) {
			nearest = moment.date;
		}
		return nearest;
	}, null);
		...
		$.fn.tick = function() {
			...
			if (isConstructor(parameters)) {
				return this.each(function(){
					Tick.DOM.create(this, parameters[0]);
				});
			}
			else {
